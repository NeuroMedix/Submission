...
def make_graph(mapidx_list, labelmap_dict, rev_labelmap_dict, labels, bondtype_list):
    ...
    for i, (smi, fragidx) in enumerate(mapidx_list):
        if smi in labelmap_dict and labelmap_dict[smi][fragidx]:
            for l in labelmap_dict[smi][fragidx].values():
                for idx in l:
                    map_dict[i].append(idx)
            mg.add_nodes(1)
        else:
            # If the mapping is empty, still add a node
            print(f"Adding node for {smi} without mapping.")
            mg.add_nodes(1)
    if len(map_dict) == 0:
        mg.add_nodes(1)
        ...
        mg = make_subtree(mg)
        # Inside make_graph function in utils.py, before the assertion
        print("Debug: mg.number_of_nodes() =", mg.number_of_nodes())
        print("Debug: len(mapidx_list) =", len(mapidx_list))
